######################################################################################################################
# EIC CORE detector model Version 2022
# Built in part from the EIC detector matrix card and allsilicon card, with changes suitable for the CORE detector
# (previously based on parameters from EIC detector matrix from EIC yellow report https://physdiv.jlab.org/DetectorMatrix/ (also in https://arxiv.org/abs/2103.05419).)
# as well as on assumptions on calorimeter granularity, and efficiency.
# EIC matrix detector email: miguel.arratia@ucr.edu, ssekula@smu.edu


# Currently based on parameters from EIC CORE detector proposal
# CORE detector implementation email: joseph.grassi@temple.edu
# Assumptions and work yet to be done:
# KLM calorimeter and parametrization
# barrelDIRC and dRICH use allsilicon parameterizations, with the efficiencies simply extended, with the same values kept.
# Tracking efficiency is parametrized assuming 1/B dependencies
# Track smearing for all-silicon is based on fits to Ernst Sichtermann GEANT4 study of CORE Proposal configuration.  This replaces the July 2021 Rey Cruz Torres parameterization


### Magnetic field parametrization:
#Simply copy the relevant text blocks from the core_parameters_file into the CommonTrackingResolution and D0 and DZ ResolutionFormula sections of this file
#As well as change the set Bz value in the particle propagator module to the desired B field value

#######################################################################################################################
# Additional updates from Delphes_EIC_CORE_3T_v6.tcl
#  KLM Muon efficiency and pi-->mu, K-->mu mis-ID implemented based on BELLE figure in CORE proposal.
#  Adjusted minimum pt vs eta for tracking efficiency
#######################################
...
set CommonTrackingEfficiency {
    ...
    (abs(eta) <= 1.0) * (pt > 0.250)                     * (1.0) +
    (abs(eta) > 1.0 && abs(eta) <= 1.5) * (pt > 0.220)   * (1.0) +
    (abs(eta) > 1.5 && abs(eta) <= 2.0) * (pt > 0.200)   * (1.0) +
    (abs(eta) > 2.0 && abs(eta) <= 2.5) * (pt > 0.150)   * (1.0) +
    (abs(eta) > 2.5 && abs(eta) <= 3.5) * (pt > 0.100)   * (1.0) +
    (abs(eta) > 3.5 && abs(eta) <= 4.0) * (pt > 0.150)   * (0.0) +
    (abs(eta) > 4.0)                                     * (0.00)+
    0.0
}

#Berkeley all-silicon tracker 3.0 T.
# Update described in https://eic.jlab.org/core/index.php/Tracking 5-Jan-2022
set CommonTrackingResolution {
    (abs(eta)<=3.5)   * (  0.0047 + pt*cosh(eta)*(1.14e-4) + (pt*cosh(eta))^2*(7.84e-7)
                    - eta^2*(8.4e-4 + pt*cosh(eta)*2.7e-5 + (pt*cosh(eta))^2 * 1.14e-7)
                    + eta^4*(2.59e-4 + pt*cosh(eta)*1.77e-6 + (pt*cosh(eta))^2 * 2.02e-7)
                    )
}
...
# Updated all-silicon tracker for 3.0T field. Improved vertex reconstruction from E. Sichtermann
# resolution in mm
  set D0ResolutionFormula "
    (abs(eta)<=3.5)                   * (sqrt( (0.0030)^2 +   (0.0273/(pt*cosh(eta))*(cosh(eta))^2.96)^2   ) )
  "


  set DZResolutionFormula "
    (abs(eta)<=3.5)                   * (sqrt( (0.0020)^2 +   (0.025/pt*(cosh(eta))^4)^2  ) )
  "
...
#  set ApplyToPileUp true
  ...
  #D0 = particle transverse impact parameter (function of eta and pt)
# Updated all-silicon tracker for 3.0T field. Improved vertex reconstruction from E. Sichtermann
# resolution in mm
  set D0ResolutionFormula "
    (abs(eta)<=3.5)                   * (sqrt( (0.0030)^2 +   (0.0273/(pt*cosh(eta))*(cosh(eta))^2.96)^2   ) )
...
module SimpleCalorimeter ECal {
  ...
  ##BARREL. For CORE, barrel extent is approximately -1.8 to 1.4.
  ##Electron and ion going sides have different cell sizes, but these are left out in this version
  #assume 0.1 x 0.1 (real cell size will be smaller, so this is to represent some cluster)

    ## Coverage is -3.5, -1.8 , and +1.4 to 3.5.
    ## Changed for CORE 2deg bins in barrel
    ...
    for {set i -36} {$i <=28} {incr i} {
	   set eta [expr {$i * 0.05}]
	   add EtaPhiBins $eta $PhiBins
    }

    for {set i 1} {$i <=23} {incr i} {
	   set eta [expr {1.35 + $i*0.05 }]
	   add EtaPhiBins $eta $PhiBins
    }
    set PhiBins {}
    for {set i -30} {$i <=30} {incr i} {
 ...
 # add EnergyFraction {3122} {0.3}
  ...
  set ResolutionFormula { (eta <= -1.8 && eta > -3.5) * sqrt((energy*0.01)^2 + energy*0.02^2 + 0.01^2) + \
            (eta <= -0.0 && eta > -1.8)    * sqrt((energy*0.01)^2 + energy*0.020^2 + 0.01^2) + \
            (eta <= 1.4  && eta > 0.0)   * sqrt((energy*0.02)^2 + energy*0.063^2 + 0.02^2)+ \
            (eta <= 3.5  &&  eta > 1.4)  * sqrt((energy*0.02)^2 + energy*0.063^2 + 0.02^2)}

}
#the above may need to be changed. Set these to PWO and W-Shashlyk efficiencies when known.
#Edited by C.Hyde 10/31/2021
...
module SimpleCalorimeter HCal {
  ...
  set pi [expr {acos(-1)}]
  ## Nominally the HCAL extent in CORE: Barrel 1<eta<1.68
  ## Until KLM parametrization though, will use full HCAL similar to allsilicon card.
  ##Match HCal towers to ECal
    set PhiBins {}
    for {set i -30} {$i <=30} {incr i} {
       add PhiBins [expr {$i * $pi/30.0}]
    }
  ...
  add EnergyFraction {1000035} {0.0}
  add EnergyFraction {1000045} {0.0}
  # energy fractions for K0short, K0Long and Lambda
  add EnergyFraction {130} {1.0}
  add EnergyFraction {310} {1.0}
  add EnergyFraction {3122} {1.0}

  # set HCalResolutionFormula {resolution formula as a function of eta and energy}
  set ResolutionFormula {    (eta <= -1.7 && eta>-3.5)                       * sqrt(energy^2*0.10^2 + energy*2.00^2)+
                             (eta <= 1.7 && eta>-1.7 )                       * sqrt(energy^2*0.10^2 + energy*2.00^2)+
                             (eta <= 3.5 && eta>1.2 )                       * sqrt(energy^2*0.05^2 + energy*0.50^2)
  }
...
module Efficiency PhotonEfficiency {
  ...
  # efficiency formula for photons
    set EfficiencyFormula { (abs(eta)<3.5)*1.0}
}
...
module Efficiency ElectronEfficiency {
  ...
  # efficiency formula for electrons
    set EfficiencyFormula {(abs(eta)<3.5)*1.0 + (eta<-6.0)*1.0}
}
...
#    Eta coverage =  [-1,1] #For CORE: [-2,1.68]
#    Assumed time precision = 0.1 ns
#    Assumed track resolution = 0.5 mrad
#    Assumed quantum efficiency of the MCP-PMT = 27%


#Efficiency in region where eta<-1 or eta>1 is taken to be same as efficiency at eta= -1 or +1 respectively.
#Long term, this should be replaced with a proper parametrization for these eta values
...
module IdentificationMap barrelDIRC {
  ...
  add EfficiencyFormula {321} {321} {
    (eta<-2.00 || eta>=1.68 || pt * cosh(eta) < 0.10 || pt * cosh(eta) >= 50.00) * ( 0.00 ) +
    (-2.00 <= eta && eta < -1.00) * (0.10 <= pt * cosh(eta) && pt * cosh(eta) < 0.20) * (0.000000) +
    (-2.00 <= eta && eta < -1.00) * (0.20 <= pt * cosh(eta) && pt * cosh(eta) < 5.70) * (0.999363) +
    (-2.00 <= eta && eta < -1.00) * (5.70 <= pt * cosh(eta) && pt * cosh(eta) < 6.20) * (0.977469) +
    (-2.00 <= eta && eta < -1.00) * (6.20 <= pt * cosh(eta) && pt * cosh(eta) < 6.60) * (0.953824) +
    (-2.00 <= eta && eta < -1.00) * (6.60 <= pt * cosh(eta) && pt * cosh(eta) < 6.90) * (0.930396) +
    (-2.00 <= eta && eta < -1.00) * (6.90 <= pt * cosh(eta) && pt * cosh(eta) < 7.20) * (0.908335) +
    (-2.00 <= eta && eta < -1.00) * (7.20 <= pt * cosh(eta) && pt * cosh(eta) < 7.50) * (0.885172) +
    (-2.00 <= eta && eta < -1.00) * (7.50 <= pt * cosh(eta) && pt * cosh(eta) < 7.80) * (0.861520) +
    (-2.00 <= eta && eta < -1.00) * (7.80 <= pt * cosh(eta) && pt * cosh(eta) < 8.10) * (0.838567) +
    (-2.00 <= eta && eta < -1.00) * (8.10 <= pt * cosh(eta) && pt * cosh(eta) < 8.40) * (0.817634) +
    (-2.00 <= eta && eta < -1.00) * (8.40 <= pt * cosh(eta) && pt * cosh(eta) < 8.80) * (0.793978) +
    (-2.00 <= eta && eta < -1.00) * (8.80 <= pt * cosh(eta) && pt * cosh(eta) < 9.20) * (0.769187) +
    (-2.00 <= eta && eta < -1.00) * (9.20 <= pt * cosh(eta) && pt * cosh(eta) < 9.70) * (0.745592) +
    (-2.00 <= eta && eta < -1.00) * (9.70 <= pt * cosh(eta) && pt * cosh(eta) < 10.30) * (0.721481) +
    (-2.00 <= eta && eta < -1.00) * (10.30 <= pt * cosh(eta) && pt * cosh(eta) < 11.00) * (0.698201) +
    (-2.00 <= eta && eta < -1.00) * (11.00 <= pt * cosh(eta) && pt * cosh(eta) < 11.90) * (0.676260) +
    (-2.00 <= eta && eta < -1.00) * (11.90 <= pt * cosh(eta) && pt * cosh(eta) < 13.30) * (0.654166) +
    (-2.00 <= eta && eta < -1.00) * (13.30 <= pt * cosh(eta) && pt * cosh(eta) < 16.50) * (0.629819) +
    (-2.00 <= eta && eta < -1.00) * (16.50 <= pt * cosh(eta) && pt * cosh(eta) < 50.00) * (0.604030) +
    ...
    (1.00 <= eta && eta < 1.68) * (0.10 <= pt * cosh(eta) && pt * cosh(eta) < 0.20) * (0.000000) +
    (1.00 <= eta && eta < 1.68) * (0.20 <= pt * cosh(eta) && pt * cosh(eta) < 5.50) * (0.999490) +
    (1.00 <= eta && eta < 1.68) * (5.50 <= pt * cosh(eta) && pt * cosh(eta) < 6.00) * (0.980505) +
    (1.00 <= eta && eta < 1.68) * (6.00 <= pt * cosh(eta) && pt * cosh(eta) < 6.40) * (0.957300) +
    (1.00 <= eta && eta < 1.68) * (6.40 <= pt * cosh(eta) && pt * cosh(eta) < 6.70) * (0.933891) +
    (1.00 <= eta && eta < 1.68) * (6.70 <= pt * cosh(eta) && pt * cosh(eta) < 7.00) * (0.910870) +
    (1.00 <= eta && eta < 1.68) * (7.00 <= pt * cosh(eta) && pt * cosh(eta) < 7.30) * (0.887137) +
    (1.00 <= eta && eta < 1.68) * (7.30 <= pt * cosh(eta) && pt * cosh(eta) < 7.60) * (0.863393) +
    (1.00 <= eta && eta < 1.68) * (7.60 <= pt * cosh(eta) && pt * cosh(eta) < 7.90) * (0.840634) +
    (1.00 <= eta && eta < 1.68) * (7.90 <= pt * cosh(eta) && pt * cosh(eta) < 8.20) * (0.818273) +
    (1.00 <= eta && eta < 1.68) * (8.20 <= pt * cosh(eta) && pt * cosh(eta) < 8.60) * (0.794422) +
    (1.00 <= eta && eta < 1.68) * (8.60 <= pt * cosh(eta) && pt * cosh(eta) < 9.00) * (0.769639) +
    (1.00 <= eta && eta < 1.68) * (9.00 <= pt * cosh(eta) && pt * cosh(eta) < 9.40) * (0.748157) +
    (1.00 <= eta && eta < 1.68) * (9.40 <= pt * cosh(eta) && pt * cosh(eta) < 10.00) * (0.724553) +
    (1.00 <= eta && eta < 1.68) * (10.00 <= pt * cosh(eta) && pt * cosh(eta) < 10.70) * (0.699984) +
    (1.00 <= eta && eta < 1.68) * (10.70 <= pt * cosh(eta) && pt * cosh(eta) < 11.60) * (0.677138) +
    (1.00 <= eta && eta < 1.68) * (11.60 <= pt * cosh(eta) && pt * cosh(eta) < 13.00) * (0.654326) +
    (1.00 <= eta && eta < 1.68) * (13.00 <= pt * cosh(eta) && pt * cosh(eta) < 16.20) * (0.629520) +
    (1.00 <= eta && eta < 1.68) * (16.20 <= pt * cosh(eta) && pt * cosh(eta) < 50.00) * (0.603898) +
    (1.68 <= eta && eta < 1.78) * (0.10 <= pt * cosh(eta) && pt * cosh(eta) < 50.00) * (0.000000)
    }

  add EfficiencyFormula {321} {211} {
    (eta<-2.00 || eta>=1.68 || pt * cosh(eta) < 0.10 || pt * cosh(eta) >= 50.00) * ( 0.00 ) +
    (-2.00 <= eta && eta < -1.00) * (0.10 <= pt * cosh(eta) && pt * cosh(eta) < 6.10) * (0.000629) +
    (-2.00 <= eta && eta < -1.00) * (6.10 <= pt * cosh(eta) && pt * cosh(eta) < 6.80) * (0.020818) +
    (-2.00 <= eta && eta < -1.00) * (6.80 <= pt * cosh(eta) && pt * cosh(eta) < 7.30) * (0.043238) +
    (-2.00 <= eta && eta < -1.00) * (7.30 <= pt * cosh(eta) && pt * cosh(eta) < 7.70) * (0.064987) +
    (-2.00 <= eta && eta < -1.00) * (7.70 <= pt * cosh(eta) && pt * cosh(eta) < 8.10) * (0.086643) +
    (-2.00 <= eta && eta < -1.00) * (8.10 <= pt * cosh(eta) && pt * cosh(eta) < 8.50) * (0.108246) +
    (-2.00 <= eta && eta < -1.00) * (8.50 <= pt * cosh(eta) && pt * cosh(eta) < 8.90) * (0.131038) +
    (-2.00 <= eta && eta < -1.00) * (8.90 <= pt * cosh(eta) && pt * cosh(eta) < 9.30) * (0.153265) +
    (-2.00 <= eta && eta < -1.00) * (9.30 <= pt * cosh(eta) && pt * cosh(eta) < 9.70) * (0.173948) +
    (-2.00 <= eta && eta < -1.00) * (9.70 <= pt * cosh(eta) && pt * cosh(eta) < 10.20) * (0.196026) +
    (-2.00 <= eta && eta < -1.00) * (10.20 <= pt * cosh(eta) && pt * cosh(eta) < 10.70) * (0.219035) +
    (-2.00 <= eta && eta < -1.00) * (10.70 <= pt * cosh(eta) && pt * cosh(eta) < 11.30) * (0.242086) +
    (-2.00 <= eta && eta < -1.00) * (11.30 <= pt * cosh(eta) && pt * cosh(eta) < 11.90) * (0.264737) +
    (-2.00 <= eta && eta < -1.00) * (11.90 <= pt * cosh(eta) && pt * cosh(eta) < 12.60) * (0.286490) +
    (-2.00 <= eta && eta < -1.00) * (12.60 <= pt * cosh(eta) && pt * cosh(eta) < 13.40) * (0.308418) +
    (-2.00 <= eta && eta < -1.00) * (13.40 <= pt * cosh(eta) && pt * cosh(eta) < 14.30) * (0.329730) +
    (-2.00 <= eta && eta < -1.00) * (14.30 <= pt * cosh(eta) && pt * cosh(eta) < 15.40) * (0.350771) +
    (-2.00 <= eta && eta < -1.00) * (15.40 <= pt * cosh(eta) && pt * cosh(eta) < 16.80) * (0.372200) +
    (-2.00 <= eta && eta < -1.00) * (16.80 <= pt * cosh(eta) && pt * cosh(eta) < 18.60) * (0.393662) +
    (-2.00 <= eta && eta < -1.00) * (18.60 <= pt * cosh(eta) && pt * cosh(eta) < 21.20) * (0.415409) +
    (-2.00 <= eta && eta < -1.00) * (21.20 <= pt * cosh(eta) && pt * cosh(eta) < 25.30) * (0.437641) +
    (-2.00 <= eta && eta < -1.00) * (25.30 <= pt * cosh(eta) && pt * cosh(eta) < 34.20) * (0.461287) +
    (-2.00 <= eta && eta < -1.00) * (34.20 <= pt * cosh(eta) && pt * cosh(eta) < 50.00) * (0.480405) +
    ...
    (1.00 <= eta && eta < 1.68) * (0.10 <= pt * cosh(eta) && pt * cosh(eta) < 6.00) * (0.000687) +
    (1.00 <= eta && eta < 1.68) * (6.00 <= pt * cosh(eta) && pt * cosh(eta) < 6.60) * (0.020501) +
    (1.00 <= eta && eta < 1.68) * (6.60 <= pt * cosh(eta) && pt * cosh(eta) < 7.10) * (0.041727) +
    (1.00 <= eta && eta < 1.68) * (7.10 <= pt * cosh(eta) && pt * cosh(eta) < 7.50) * (0.063800) +
    (1.00 <= eta && eta < 1.68) * (7.50 <= pt * cosh(eta) && pt * cosh(eta) < 7.90) * (0.084994) +
    (1.00 <= eta && eta < 1.68) * (7.90 <= pt * cosh(eta) && pt * cosh(eta) < 8.30) * (0.107765) +
    (1.00 <= eta && eta < 1.68) * (8.30 <= pt * cosh(eta) && pt * cosh(eta) < 8.70) * (0.130564) +
    (1.00 <= eta && eta < 1.68) * (8.70 <= pt * cosh(eta) && pt * cosh(eta) < 9.10) * (0.152720) +
    (1.00 <= eta && eta < 1.68) * (9.10 <= pt * cosh(eta) && pt * cosh(eta) < 9.50) * (0.174241) +
    (1.00 <= eta && eta < 1.68) * (9.50 <= pt * cosh(eta) && pt * cosh(eta) < 10.00) * (0.197247) +
    (1.00 <= eta && eta < 1.68) * (10.00 <= pt * cosh(eta) && pt * cosh(eta) < 10.50) * (0.220892) +
    (1.00 <= eta && eta < 1.68) * (10.50 <= pt * cosh(eta) && pt * cosh(eta) < 11.00) * (0.242281) +
    (1.00 <= eta && eta < 1.68) * (11.00 <= pt * cosh(eta) && pt * cosh(eta) < 11.60) * (0.263564) +
    (1.00 <= eta && eta < 1.68) * (11.60 <= pt * cosh(eta) && pt * cosh(eta) < 12.30) * (0.285942) +
    (1.00 <= eta && eta < 1.68) * (12.30 <= pt * cosh(eta) && pt * cosh(eta) < 13.10) * (0.308428) +
    (1.00 <= eta && eta < 1.68) * (13.10 <= pt * cosh(eta) && pt * cosh(eta) < 14.00) * (0.330204) +
    (1.00 <= eta && eta < 1.68) * (14.00 <= pt * cosh(eta) && pt * cosh(eta) < 15.10) * (0.351616) +
    (1.00 <= eta && eta < 1.68) * (15.10 <= pt * cosh(eta) && pt * cosh(eta) < 16.50) * (0.373325) +
    (1.00 <= eta && eta < 1.68) * (16.50 <= pt * cosh(eta) && pt * cosh(eta) < 18.30) * (0.394961) +
    (1.00 <= eta && eta < 1.68) * (18.30 <= pt * cosh(eta) && pt * cosh(eta) < 20.90) * (0.416763) +
    (1.00 <= eta && eta < 1.68) * (20.90 <= pt * cosh(eta) && pt * cosh(eta) < 25.10) * (0.439153) +
    (1.00 <= eta && eta < 1.68) * (25.10 <= pt * cosh(eta) && pt * cosh(eta) < 34.50) * (0.463089) +
    (1.00 <= eta && eta < 1.68) * (34.50 <= pt * cosh(eta) && pt * cosh(eta) < 50.00) * (0.481465) +
    (1.68 <= eta && eta < 1.78) * (0.10 <= pt * cosh(eta) && pt * cosh(eta) < 50.00) * (0.000000) }

  add EfficiencyFormula {321} {2212} {
    (eta<-2.00 || eta>=1.68 || pt * cosh(eta) < 0.10 || pt * cosh(eta) >= 50.00) * ( 0.00 ) +
    (-2.00 <= eta && eta < -1.00) * (0.10 <= pt * cosh(eta) && pt * cosh(eta) < 10.30) * (0.000705) +
    (-2.00 <= eta && eta < -1.00) * (10.30 <= pt * cosh(eta) && pt * cosh(eta) < 11.40) * (0.021025) +
    (-2.00 <= eta && eta < -1.00) * (11.40 <= pt * cosh(eta) && pt * cosh(eta) < 12.20) * (0.042481) +
    (-2.00 <= eta && eta < -1.00) * (12.20 <= pt * cosh(eta) && pt * cosh(eta) < 12.90) * (0.063884) +
    (-2.00 <= eta && eta < -1.00) * (12.90 <= pt * cosh(eta) && pt * cosh(eta) < 13.60) * (0.085969) +
    (-2.00 <= eta && eta < -1.00) * (13.60 <= pt * cosh(eta) && pt * cosh(eta) < 14.30) * (0.108965) +
    (-2.00 <= eta && eta < -1.00) * (14.30 <= pt * cosh(eta) && pt * cosh(eta) < 15.00) * (0.132030) +
    (-2.00 <= eta && eta < -1.00) * (15.00 <= pt * cosh(eta) && pt * cosh(eta) < 15.70) * (0.154564) +
    (-2.00 <= eta && eta < -1.00) * (15.70 <= pt * cosh(eta) && pt * cosh(eta) < 16.40) * (0.176174) +
    (-2.00 <= eta && eta < -1.00) * (16.40 <= pt * cosh(eta) && pt * cosh(eta) < 17.20) * (0.198024) +
    (-2.00 <= eta && eta < -1.00) * (17.20 <= pt * cosh(eta) && pt * cosh(eta) < 18.00) * (0.219729) +
    (-2.00 <= eta && eta < -1.00) * (18.00 <= pt * cosh(eta) && pt * cosh(eta) < 18.90) * (0.240906) +
    (-2.00 <= eta && eta < -1.00) * (18.90 <= pt * cosh(eta) && pt * cosh(eta) < 19.90) * (0.262363) +
    (-2.00 <= eta && eta < -1.00) * (19.90 <= pt * cosh(eta) && pt * cosh(eta) < 21.00) * (0.283549) +
    (-2.00 <= eta && eta < -1.00) * (21.00 <= pt * cosh(eta) && pt * cosh(eta) < 22.30) * (0.304817) +
    (-2.00 <= eta && eta < -1.00) * (22.30 <= pt * cosh(eta) && pt * cosh(eta) < 23.80) * (0.326209) +
    (-2.00 <= eta && eta < -1.00) * (23.80 <= pt * cosh(eta) && pt * cosh(eta) < 25.60) * (0.347442) +
    (-2.00 <= eta && eta < -1.00) * (25.60 <= pt * cosh(eta) && pt * cosh(eta) < 27.80) * (0.368549) +
    (-2.00 <= eta && eta < -1.00) * (27.80 <= pt * cosh(eta) && pt * cosh(eta) < 30.70) * (0.389801) +
    (-2.00 <= eta && eta < -1.00) * (30.70 <= pt * cosh(eta) && pt * cosh(eta) < 34.70) * (0.411336) +
    (-2.00 <= eta && eta < -1.00) * (34.70 <= pt * cosh(eta) && pt * cosh(eta) < 40.90) * (0.433234) +
    (-2.00 <= eta && eta < -1.00) * (40.90 <= pt * cosh(eta) && pt * cosh(eta) < 50.00) * (0.453570) +
    ...
    (1.00 <= eta && eta < 1.68) * (0.10 <= pt * cosh(eta) && pt * cosh(eta) < 10.00) * (0.000631) +
    (1.00 <= eta && eta < 1.68) * (10.00 <= pt * cosh(eta) && pt * cosh(eta) < 11.10) * (0.020111) +
    (1.00 <= eta && eta < 1.68) * (11.10 <= pt * cosh(eta) && pt * cosh(eta) < 11.90) * (0.041754) +
    (1.00 <= eta && eta < 1.68) * (11.90 <= pt * cosh(eta) && pt * cosh(eta) < 12.60) * (0.063581) +
    (1.00 <= eta && eta < 1.68) * (12.60 <= pt * cosh(eta) && pt * cosh(eta) < 13.20) * (0.084516) +
    (1.00 <= eta && eta < 1.68) * (13.20 <= pt * cosh(eta) && pt * cosh(eta) < 13.80) * (0.104653) +
    (1.00 <= eta && eta < 1.68) * (13.80 <= pt * cosh(eta) && pt * cosh(eta) < 14.40) * (0.124945) +
    (1.00 <= eta && eta < 1.68) * (14.40 <= pt * cosh(eta) && pt * cosh(eta) < 15.10) * (0.146583) +
    (1.00 <= eta && eta < 1.68) * (15.10 <= pt * cosh(eta) && pt * cosh(eta) < 15.80) * (0.169067) +
    (1.00 <= eta && eta < 1.68) * (15.80 <= pt * cosh(eta) && pt * cosh(eta) < 16.50) * (0.190404) +
    (1.00 <= eta && eta < 1.68) * (16.50 <= pt * cosh(eta) && pt * cosh(eta) < 17.30) * (0.211798) +
    (1.00 <= eta && eta < 1.68) * (17.30 <= pt * cosh(eta) && pt * cosh(eta) < 18.20) * (0.234147) +
    (1.00 <= eta && eta < 1.68) * (18.20 <= pt * cosh(eta) && pt * cosh(eta) < 19.10) * (0.255662) +
    (1.00 <= eta && eta < 1.68) * (19.10 <= pt * cosh(eta) && pt * cosh(eta) < 20.20) * (0.277068) +
    (1.00 <= eta && eta < 1.68) * (20.20 <= pt * cosh(eta) && pt * cosh(eta) < 21.40) * (0.298797) +
    (1.00 <= eta && eta < 1.68) * (21.40 <= pt * cosh(eta) && pt * cosh(eta) < 22.80) * (0.320057) +
    (1.00 <= eta && eta < 1.68) * (22.80 <= pt * cosh(eta) && pt * cosh(eta) < 24.50) * (0.341541) +
    (1.00 <= eta && eta < 1.68) * (24.50 <= pt * cosh(eta) && pt * cosh(eta) < 26.60) * (0.363233) +
    (1.00 <= eta && eta < 1.68) * (26.60 <= pt * cosh(eta) && pt * cosh(eta) < 29.20) * (0.384597) +
    (1.00 <= eta && eta < 1.68) * (29.20 <= pt * cosh(eta) && pt * cosh(eta) < 32.80) * (0.405989) +
    (1.00 <= eta && eta < 1.68) * (32.80 <= pt * cosh(eta) && pt * cosh(eta) < 38.20) * (0.427888) +
    (1.00 <= eta && eta < 1.68) * (38.20 <= pt * cosh(eta) && pt * cosh(eta) < 48.00) * (0.450605) +
    (1.00 <= eta && eta < 1.68) * (48.00 <= pt * cosh(eta) && pt * cosh(eta) < 50.00) * (0.462220) +
    (1.68 <= eta && eta < 1.78) * (0.10 <= pt * cosh(eta) && pt * cosh(eta) < 50.00) * (0.000000) }
  ...
  add EfficiencyFormula {2212} {2212} {
    (eta<-2.00 || eta>=1.68 || pt * cosh(eta) < 0.10 || pt * cosh(eta) >= 50.00) * ( 0.00 ) +
    (-2.00 <= eta && eta < -1.00) * (0.10 <= pt * cosh(eta) && pt * cosh(eta) < 0.60) * (0.000000) +
    (-2.00 <= eta && eta < -1.00) * (0.60 <= pt * cosh(eta) && pt * cosh(eta) < 9.50) * (0.999393) +
    (-2.00 <= eta && eta < -1.00) * (9.50 <= pt * cosh(eta) && pt * cosh(eta) < 10.30) * (0.979966) +
    (-2.00 <= eta && eta < -1.00) * (10.30 <= pt * cosh(eta) && pt * cosh(eta) < 10.90) * (0.959319) +
    (-2.00 <= eta && eta < -1.00) * (10.90 <= pt * cosh(eta) && pt * cosh(eta) < 11.40) * (0.938208) +
    (-2.00 <= eta && eta < -1.00) * (11.40 <= pt * cosh(eta) && pt * cosh(eta) < 11.90) * (0.916518) +
    (-2.00 <= eta && eta < -1.00) * (11.90 <= pt * cosh(eta) && pt * cosh(eta) < 12.40) * (0.893591) +
    (-2.00 <= eta && eta < -1.00) * (12.40 <= pt * cosh(eta) && pt * cosh(eta) < 12.90) * (0.870367) +
    (-2.00 <= eta && eta < -1.00) * (12.90 <= pt * cosh(eta) && pt * cosh(eta) < 13.40) * (0.847581) +
    (-2.00 <= eta && eta < -1.00) * (13.40 <= pt * cosh(eta) && pt * cosh(eta) < 13.90) * (0.825759) +
    (-2.00 <= eta && eta < -1.00) * (13.90 <= pt * cosh(eta) && pt * cosh(eta) < 14.50) * (0.803286) +
    (-2.00 <= eta && eta < -1.00) * (14.50 <= pt * cosh(eta) && pt * cosh(eta) < 15.10) * (0.780814) +
    (-2.00 <= eta && eta < -1.00) * (15.10 <= pt * cosh(eta) && pt * cosh(eta) < 15.80) * (0.759031) +
    (-2.00 <= eta && eta < -1.00) * (15.80 <= pt * cosh(eta) && pt * cosh(eta) < 16.60) * (0.737077) +
    (-2.00 <= eta && eta < -1.00) * (16.60 <= pt * cosh(eta) && pt * cosh(eta) < 17.60) * (0.714871) +
    (-2.00 <= eta && eta < -1.00) * (17.60 <= pt * cosh(eta) && pt * cosh(eta) < 18.80) * (0.692935) +
    (-2.00 <= eta && eta < -1.00) * (18.80 <= pt * cosh(eta) && pt * cosh(eta) < 20.50) * (0.671054) +
    (-2.00 <= eta && eta < -1.00) * (20.50 <= pt * cosh(eta) && pt * cosh(eta) < 23.20) * (0.648575) +
    (-2.00 <= eta && eta < -1.00) * (23.20 <= pt * cosh(eta) && pt * cosh(eta) < 32.00) * (0.621769) +
    (-2.00 <= eta && eta < -1.00) * (32.00 <= pt * cosh(eta) && pt * cosh(eta) < 50.00) * (0.605753) +
    ...
    (1.00 <= eta && eta < 1.68) * (0.10 <= pt * cosh(eta) && pt * cosh(eta) < 0.60) * (0.000000) +
    (1.00 <= eta && eta < 1.68) * (0.60 <= pt * cosh(eta) && pt * cosh(eta) < 9.30) * (0.999390) +
    (1.00 <= eta && eta < 1.68) * (9.30 <= pt * cosh(eta) && pt * cosh(eta) < 10.10) * (0.979355) +
    (1.00 <= eta && eta < 1.68) * (10.10 <= pt * cosh(eta) && pt * cosh(eta) < 10.70) * (0.957686) +
    (1.00 <= eta && eta < 1.68) * (10.70 <= pt * cosh(eta) && pt * cosh(eta) < 11.20) * (0.935743) +
    (1.00 <= eta && eta < 1.68) * (11.20 <= pt * cosh(eta) && pt * cosh(eta) < 11.70) * (0.913316) +
    (1.00 <= eta && eta < 1.68) * (11.70 <= pt * cosh(eta) && pt * cosh(eta) < 12.20) * (0.889743) +
    (1.00 <= eta && eta < 1.68) * (12.20 <= pt * cosh(eta) && pt * cosh(eta) < 12.70) * (0.866001) +
    (1.00 <= eta && eta < 1.68) * (12.70 <= pt * cosh(eta) && pt * cosh(eta) < 13.20) * (0.842837) +
    (1.00 <= eta && eta < 1.68) * (13.20 <= pt * cosh(eta) && pt * cosh(eta) < 13.70) * (0.820773) +
    (1.00 <= eta && eta < 1.68) * (13.70 <= pt * cosh(eta) && pt * cosh(eta) < 14.30) * (0.798174) +
    (1.00 <= eta && eta < 1.68) * (14.30 <= pt * cosh(eta) && pt * cosh(eta) < 14.90) * (0.775697) +
    (1.00 <= eta && eta < 1.68) * (14.90 <= pt * cosh(eta) && pt * cosh(eta) < 15.60) * (0.754029) +
    (1.00 <= eta && eta < 1.68) * (15.60 <= pt * cosh(eta) && pt * cosh(eta) < 16.40) * (0.732311) +
    (1.00 <= eta && eta < 1.68) * (16.40 <= pt * cosh(eta) && pt * cosh(eta) < 17.40) * (0.710471) +
    (1.00 <= eta && eta < 1.68) * (17.40 <= pt * cosh(eta) && pt * cosh(eta) < 18.70) * (0.688204) +
    (1.00 <= eta && eta < 1.68) * (18.70 <= pt * cosh(eta) && pt * cosh(eta) < 20.50) * (0.665960) +
    (1.00 <= eta && eta < 1.68) * (20.50 <= pt * cosh(eta) && pt * cosh(eta) < 23.60) * (0.643220) +
    (1.00 <= eta && eta < 1.68) * (23.60 <= pt * cosh(eta) && pt * cosh(eta) < 50.00) * (0.609760) +
    (1.68 <= eta && eta < 1.78) * (0.10 <= pt * cosh(eta) && pt * cosh(eta) < 50.00) * (0.000000) }
...
#    Eta coverage =  [1.7,4] #taken from allsilicon card, with efficiency set to 0 from 1<eta<1.7
...
module IdentificationMap dualRICH_aerogel {
  ...
  add EfficiencyFormula {321} {321} {
    (eta<1.70 || eta>=3.50 || pt * cosh(eta) < 0.10 || pt * cosh(eta) >= 50.00) * ( 0.00 ) +
...
module IdentificationMap dualRICH_c2f6 {
  ...
  # Everything else is not ID'd at all.
  add EfficiencyFormula {0} {0} { 0.0 }
}

module IdentificationMap KLM {
# Extracted from BELLE plots in CORE Proposal
  set InputArray TrackSmearing/tracks
  set OutputArray tracks
  add EfficiencyFormula {13} {13} { ( (abs(eta)<3.5)&&(pt*cosh(eta)>0.40) ) * 0.93*(1.-exp(-pt*cosh(eta)/0.5))
                                  + ( (abs(eta)<3.5)&&(pt*cosh(eta)<=0.40) )*0.50 }
  add EfficiencyFormula {13} {211} { ( (abs(eta)<3.5)&&(pt*cosh(eta)>0.40) ) * 0.93*exp(-pt*cosh(eta)/0.5)
                                   + ( (abs(eta)<3.5)&&(pt*cosh(eta)<0.40) ) *0.50}
  add EfficiencyFormula {211} {13} { ( (abs(eta)<3.5)&&(pt*cosh(eta)>0.312) ) * (0.075+ 0.925*exp(-pt*cosh(eta)/0.4))
                                    + ( (abs(eta)<3.5)&&(pt*cosh(eta)<0.312) ) *0.50 }
  add EfficiencyFormula {321} {13} { (abs(eta)<3.5) * 0.075*((pt*cosh(eta)/1.4)^4/(1.0+(pt*cosh(eta)/1.4)^4)) }
  # Everything else is not ID'd at all.
  add EfficiencyFormula {0} {0} { 0.0 }
}
...
# add Branch InputArray BranchName BranchClass
  ...
  add Branch KLM/tracks KLMTrack Track
